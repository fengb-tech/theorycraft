...
const browserifyGlobalShim = require('browserify-global-shim')
...
browserifyTaskWithWatch('assets:tc.js', {
  entries: PATH.tc,
  transform: [
    reactJade({ globalReact: true }),
    babelify.configure({ externalHelpers: true }),
    browserifyGlobalShim.configure({ react: 'React', lodash: '_', eventemitter3: 'EventEmitter' }),
  ],
  run (bundle) {
    return bundle
      .bundle()

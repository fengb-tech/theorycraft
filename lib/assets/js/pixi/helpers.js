...
const helpers = module.exports = {
  create (module, options) {
    let displayObject = module.init(options)
    if (module.load) {
      for (let asset of Object.keys(module.load)) {
        let onload = module.load[asset]
        helpers.load([asset]).then((resources) => {
          onload.call(displayObject, resources)
        })
      }
    }
